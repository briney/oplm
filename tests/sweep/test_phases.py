...
def test_wandb_run_names_are_unique_across_phases(tmp_path: Path) -> None:
    """W&B run names must carry the phase, or smoke and coarse cells collide.

    `SMOKE_LRS` is a subset of `COARSE_LRS`, so the same 170M/lr/output-mult/seed cell is
    generated by both phases and shares a run id. Without the phase prefix those land in the
    W&B project under identical names, distinguishable only by step count.
    """
    base = tmp_path / "base.yaml"
    base.write_text(_base_config_text())
    names: dict[str, set[str]] = {}
    for phase in ("smoke", "coarse"):
        out = tmp_path / phase
        result = runner.invoke(phases.app, [phase, "--config", str(base), "--out", str(out)])
        assert result.exit_code == 0, result.output
        names[phase] = {
            load_config(["--config", str(run_yaml)]).train.wandb_run_name
            for run_yaml in sorted(out.glob("runs/*/run.yaml"))
        }
        assert all(n.startswith(f"{phase}-") for n in names[phase]), names[phase]

    # The bare run ids DO overlap -- that is what makes the prefix load-bearing.
    ids = {phase: {n.split("-", 1)[1] for n in phase_names} for phase, phase_names in names.items()}
    assert ids["smoke"] & ids["coarse"], "expected overlapping run ids; grids may have drifted"
    assert not (names["smoke"] & names["coarse"])
