...
- [ ] **3.2 `make_flex_block_mask(attention_mask: Tensor) -> BlockMask`**:
  - Builds a closure `mask_mod(b, h, q_idx, kv_idx) -> bool` that returns
    `attention_mask[b, kv_idx] == 1` (bidirectional encoder: no causal
    restriction; only KV padding is masked).
  - Calls `torch.nn.attention.flex_attention.create_block_mask(mask_mod,
    B=B, H=None, Q_LEN=T, KV_LEN=T, device=...)`.
  - Returns the resulting `BlockMask`. Caches by `(B, T, device)` is **not**
...
- [ ] **8.1 Implement `OplmAttention(nn.Module)`** with:
  - Constructor args derived from `OplmConfig`: `hidden_size, num_attention_heads,
    head_dim, rope_dim, max_position_embeddings, rope_theta, norm_type,
    norm_eps, qk_norm, attention_dropout, hidden_dropout, use_flex_attention`.
  - Parameters:
    - `q_proj, k_proj, v_proj`: `Linear(D, D, bias=False)` each.
    - `o_proj`: `Linear(D, D, bias=False)`.
...
- [ ] **8.3 Fast-path kernel `_flex_attention(self, q, k, v, attention_mask)`**:
  ...
  - Note: `flex_attention` returns no attention weights; the caller is
    responsible for setting `attn = None`.

- [ ] **8.4 Fallback-path kernel `_manual_attention(self, q, k, v, attention_mask)`**:
...
- [ ] **8.5 Forward**:
  ...
  def forward(self, x, attention_mask, output_attentions=False):
      ...
      if output_attentions or not self.use_flex_attention:
...
- [ ] **8.6 Tests** (`tests/model/test_attention.py`):
  - Output shape `(B, T, D)`.
...
- [ ] **9.1 Implement `OplmBlock(nn.Module)`**:
  ...
  - Sub-modules:
    - `self.attn_norm = make_norm(...)`. Always present.
    ...
    - Strategy-specific extras (each created only when needed):
      ...
      - `hybrid`: `self.ffn_post_norm` only.
...
- [ ] **9.2 Forward — pseudocode (handles every `norm_strategy` × Canon combo)**:
  ...
  def forward(self, x, attention_mask, output_attentions):
      ...
      if hasattr(self, "conv_b"):
          h_in = self.conv_b(h_in, attention_mask)
      attn_out, attn_weights = self.attention(h_in, attention_mask, output_attentions)
      ...
      if self.norm_strategy == "sandwich":
          ...
          y = self.ffn_post_norm(h + self.alpha * ffn_out)
...
- [ ] **10.1 Implement `OplmConfig(PretrainedConfig)`** with `model_type = "oplm"`.
  Constructor accepts every field from §12.1 of the architecture doc. Defaults
...
- [ ] **10.4 `auto_map` population**: override `save_pretrained` (or set
  `auto_map` in `__init__` so HF persists it via the normal serialization) to
...
- [ ] **12.4 `EsmcCompatMixin`** (defined in this file):
  - `encode(self, seqs: list[str], **tokenizer_kwargs) -> Tensor`:
    Loads its own tokenizer lazily from `self.config._name_or_path` via
    `AutoTokenizer.from_pretrained(...)`, then returns
    `tokenizer(seqs, return_tensors="pt", padding=True).input_ids` moved to
    the model's device.
  - `logits(self, seqs: list[str], config: LogitsConfig | None = None) -> LogitsOutput`:
    - Default `config = LogitsConfig()`.
    - Tokenize.
    - Call `self.forward(input_ids, attention_mask,
      output_hidden_states=(config.return_hidden_states or
      config.return_embeddings), output_attentions=config.return_attentions)`.
    - Repackage:
      - `sequence_logits`: model output `logits` if `config.sequence` else `None`.
      - `embeddings`: last hidden state if `config.return_embeddings` else `None`.
      - `hidden_states`: full tuple if `config.return_hidden_states` else `None`.
      - `attentions`: tuple if `config.return_attentions` else `None`.
  - This mixin is inherited by `OplmForMaskedLM`,
    ...
    `sequence_logits=None` because `OplmModel` has no head.
...
- [ ] **13.3 Verify the registration is idempotent** — re-importing `oplm`
  ...
  guard with `try/except` or check the registry first.)
...
- [ ] **14.5 `tests/model/test_esmc_api.py`**:
  - Build `OplmForMaskedLM`, call `model.encode(["MEEPQ"])`, assert shape and
    dtype.
  - Call `model.logits(["MEEPQ", "GAGT"], LogitsConfig(sequence=True,
    return_embeddings=True))`; assert `sequence_logits.shape == (2, T, 33)`
    and `embeddings.shape == (2, T, D)`.
...
3. **`hybrid` norm strategy** applies a single post-norm after the FFN
   residual add only (FFN sublayer's `Norm(h + α·FFN(h))`); the attention
   sublayer stays pre-norm. The block code in 9.2 reflects this. Double-check
   against arXiv 2503.04598 if the owner reports unexpected behavior.
